...
/*
 * try formulating into something binomial series, would easy to solve with O(N)
 */
public class ManasaStones {
	public static void main(String...args){
		...
		for(int i=0;i<t;i++){
			...
			int a = sc.nextInt();
			int b = sc.nextInt();
			int _n = n - 1;//since only n-1 times can we apply the a or b values.
			PriorityQueue<Integer> possibility = new PriorityQueue<>();
			for(int j=0;j<n;j++){
				Integer x = ((_n-j)*a) + (j*b);
				if(!possibility.contains(x)){
					possibility.add(x);
				}
			}
			while(!possibility.isEmpty()){
				System.out.print(possibility.poll()+" ");
			}
			System.out.println();
		}
	}
